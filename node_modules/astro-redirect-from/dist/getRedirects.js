...
function isValidDomainRelativePath(path) {
    return !path.includes('://') && !path.includes(' ') && !path.includes('\n');
}
export async function getRedirects(files, srcDir, getSlug, command, logger) {
    let redirects = {};
    for (const file of files) {
        ...
        if (!frontmatter || !redirectFrom.length || isExcluded)
            continue;
